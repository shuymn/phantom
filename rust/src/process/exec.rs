...
pub async fn spawn_shell_in_worktree(
    ...
    // Log exit
    info!(
        "Shell exited with code {} for worktree '{}'",
        result.exit_code, worktree_name
    );
...
pub async fn exec_commands_in_dir(
    dir: &Path,
    commands: &[String],
) -> Result<Vec<SpawnSuccess>> {
...
mod tests {
    ...
    async fn test_exec_in_worktree() {
        ...
        create_worktree(repo.path(), "test-worktree", options)
            .await
            .unwrap();
