...
fn bench_cloning_patterns(c: &mut Criterion) {
    ...
    let large_data = LargeData {
        data: (0..1000).map(|i| format!("Item {}", i)).collect(),
    };
...
fn bench_smallvec_optimization(c: &mut Criterion) {
    ...
    group.bench_function("vec_small", |b| {
        b.iter(|| {
            let args: Vec<String> = small_args.iter()
                .map(|s| s.to_string())
                .collect();
            black_box(args);
        });
    ...
    group.bench_function("smallvec_small", |b| {
        b.iter(|| {
            let args: SmallVec<[String; 4]> = small_args.iter()
                .map(|s| s.to_string())
                .collect();
            black_box(args);
        });
    ...
    let large_args: Vec<&str> = (0..10)
        .map(|_| "arg")
        .collect();
    
    group.bench_function("vec_large", |b| {
        b.iter(|| {
            let args: Vec<String> = large_args.iter()
                .map(|s| s.to_string())
                .collect();
            black_box(args);
        });
    ...
    group.bench_function("smallvec_large", |b| {
        b.iter(|| {
            let args: SmallVec<[String; 4]> = large_args.iter()
                .map(|s| s.to_string())
                .collect();
            black_box(args);
        });
