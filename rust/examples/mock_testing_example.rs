...
impl GitService {
    ...
    async fn commit_changes(&self, message: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let add_config = CommandConfig::new("git")
            .with_args(vec!["add".to_string(), ".".to_string()]);
        
        self.executor.execute(add_config).await?;

        // Then commit with message
        let commit_config = CommandConfig::new("git")
...
async fn main() {
    ...
    mock.expect_command("git")
        ...
        .times(1)
        .returns_output("feature/mock-testing\n", "", 0);
